...
$.ajax({
    ...
    success: function(req){//请求成功的处理
        ...
        reqData.forEach((item,index) =>{
            ...
            menuHtml += `<a data-type="${item.type}" data-request="false" href="javascript:void(0);" onclick="loadNewsData({_this:this,id:${item.id},index:${index}})" title="${item.categoryName}" class="${currentName}">${item.categoryName}</a>`
        })
        //菜单
